...
class Market:
    def __init__(self,prices,demands,supplies):
        self.prices = prices
    ...
    def plotWithGrant(self):
        ...
        self.balance = self.demandCurve.intersection(self.supplyCurve,[min(self.prices),max(self.prices)])
        balance_quantity = self.demandCurve(self.balance)
        lbl = "Old:(" + doubleToStr(self.balance,4) + ", " + doubleToStr(balance_quantity,4) + ")"
        aproximatedData.scatter(balance_quantity,self.balance,c='r',label=lbl)
        
        grant_count = 5
        for i in range(grant_count):
            color = [255*(grant_count-i)/grant_count,255*i/grant_count,0]
            self.grantCurve = Polynom(np.array(self.prices)-2*i,self.supplies)
            self.grantCurve.polynomicalApproximation(self.approximationPower)
            aproximatedData.scatter(self.grantCurve(self.xrange),self.xrange,c=grad(color,len(self.xrange),True),s=5)
            self.grantBalance = self.demandCurve.intersection(self.grantCurve,[min(self.prices),max(self.prices)])
            balance_quantity = self.demandCurve(self.grantBalance)
            lbl = "With grant:(" + doubleToStr(self.balance,4) + ", " + doubleToStr(balance_quantity,4) + ")"
            aproximatedData.scatter(balance_quantity,self.grantBalance,c='r',label=lbl)     
